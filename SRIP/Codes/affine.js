...
function scale(inputImage, toUpScale, scaleFactor, interpolType) {
    let outputImage = new ImageData(Math.round(inputImage.width * Math.pow(2, toUpScale ? scaleFactor : (-1 * scaleFactor))), Math.round(inputImage.height * Math.pow(2, toUpScale ? scaleFactor : (-1 * scaleFactor))));
    ...
    if(toUpScale) {
        for(let i = 0; i <= outputImage.data.length; i += 4) {
            ...
            if(origCoord.x % 1 === 0 && origCoord.y % 1 === 0) {
                ...
                let pixelvalues = interpolate(inputImage, currCoord.x, currCoord.y, interpolType);
        ...
        for (let i = 0; i <= outputImage.data.length; i += 4) {
            ...
            if(origCoord.x % 1 === 0 && origCoord.y % 1 === 0 && origCoord.x < inputImage.width && origCoord.y < inputImage.height) {
